...
if($link = OpenCon()) {
  ...
  /* ***************************************************************************** */
    	$query ="SELECT distinct pincode FROM localities";

      if(!empty($term) && !empty($districtid)) {
  	       $query =$query. " where districtid = '". $districtid ."' and pincode like '" . $term . "%'";
      } else if(!empty($districtid)) {
           $query =$query. " where districtid = '". $districtid ."'";
      } else if(!empty($term)) {
            $query =$query. " where pincode like '" . $term . "%'";
      }
    ...
    $x=0;
      ...
      $localityname = $locality['pincode'];
  		$a_json[$x] =$localityname;
      $x++;
  	}
